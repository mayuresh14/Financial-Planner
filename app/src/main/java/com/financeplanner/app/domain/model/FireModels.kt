...
/** Which value the FIRE calculator solves for: a required SIP for a chosen age, or the age reachable with a fixed SIP. */
enum class FireCalculationMode { FIND_SIP, FIND_AGE }

data class FireAgeInput(
    val variant: FireVariant,
    val currentAge: Int,
    val currentAnnualExpenses: Double,
    val inflationPercent: Double,
    ...
    init {
        ...
        require(inflationPercent >= 0) { "Inflation cannot be negative" }
        require(preRetirementReturnPercent >= 0) { "Expected return cannot be negative" }
        require(monthlySip > 0) { "Monthly SIP must be positive" }
        require(existingCorpus >= 0) { "Existing corpus cannot be negative" }
    }
}

data class FireAgeResult(
    val fireAge: Int?, // null if unreachable within the simulated cap
    val yearsToFire: Double?,
    val requiredCorpusAtFire: Double?
)
