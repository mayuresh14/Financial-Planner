...
fun EatingOutScreen(
    onBack: () -> Unit,
    viewModel: EatingOutViewModel = hiltViewModel(),
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.calc_eating_out)) },
        ...
        Column(
            ...
            Text(
                text = stringResource(R.string.eating_out_screen_description),
                style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )

            AmountOutlinedTextField(
                value = state.eatingOutMonthlySpend, onValueChange = viewModel::onEatingOutSpendChange,
                label = { Text(stringResource(R.string.eating_out_label_eating_out_spend)) },
                modifier = Modifier.fillMaxWidth()
            )
            AmountOutlinedTextField(
                value = state.homeCookingMonthlySpend, onValueChange = viewModel::onHomeCookingSpendChange,
                label = { Text(stringResource(R.string.eating_out_label_home_cooking_spend)) },
                modifier = Modifier.fillMaxWidth()
            )
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                OutlinedTextField(
                    value = state.years, onValueChange = viewModel::onYearsChange,
                    label = { Text(stringResource(R.string.eating_out_label_years)) },
            ...
            state.error?.let { error ->
                val message = when (error) {
                    is EatingOutValidationError.InvalidInput -> stringResource(R.string.eating_out_error_invalid_input)
                    is EatingOutValidationError.InvalidValue -> error.rawMessage ?: stringResource(R.string.eating_out_error_invalid_input)
                }
...
@Composable
private fun EatingOutResultCard(result: EatingOutResult, state: EatingOutUiState) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        val narrative = buildString {
            append(
                stringResource(
                    R.string.eating_out_result_narrative,
                    formatAmountWithWords(result.monthlySavings, currencyFormat),
                    state.years,
                    formatAmountWithWords(result.totalMoneySaved, currencyFormat),
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                Text(text = stringResource(R.string.eating_out_result_total_saved), style = MaterialTheme.typography.labelLarge)
                Text(
                    text = currencyFormat.format(result.totalMoneySaved),
