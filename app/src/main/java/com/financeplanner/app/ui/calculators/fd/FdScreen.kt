...
fun FdScreen(
    onBack: () -> Unit,
    viewModel: FdViewModel = hiltViewModel(),
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.calc_fd)) },
        ...
        Column(
            ...
            Text(
                text = stringResource(R.string.fd_screen_description),
...
@Composable
private fun FdResultCard(result: FdResult, state: FdUiState) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        val narrative = buildString {
            append(
                stringResource(
                    ...
                    formatAmountWithWords(result.totalInterestEarned, currencyFormat)
                )
            )
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                Text(text = stringResource(R.string.fd_result_maturity_value), style = MaterialTheme.typography.labelLarge)
                Text(
                    text = currencyFormat.format(result.maturityValue),
                    style = MaterialTheme.typography.headlineMedium, fontWeight = FontWeight.Bold
                )
                HorizontalDivider(modifier = Modifier.padding(vertical = 4.dp))
                ResultRow(stringResource(R.string.fd_result_interest_earned), currencyFormat.format(result.totalInterestEarned))
