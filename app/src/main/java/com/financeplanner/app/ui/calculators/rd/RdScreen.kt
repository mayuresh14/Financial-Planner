...
fun RdScreen(
    onBack: () -> Unit,
    viewModel: RdViewModel = hiltViewModel(),
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.calc_rd)) },
        ...
        Column(
            ...
            Text(
                text = stringResource(R.string.rd_screen_description),
                style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )

            AmountOutlinedTextField(
                value = state.monthlyDeposit, onValueChange = viewModel::onDepositChange,
                label = { Text(stringResource(R.string.rd_label_deposit)) },
                modifier = Modifier.fillMaxWidth()
            )
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                OutlinedTextField(
                    value = state.tenureMonths, onValueChange = viewModel::onTenureChange,
                    label = { Text(stringResource(R.string.rd_label_tenure)) },
                    modifier = Modifier.width(180.dp), singleLine = true
                )
                OutlinedTextField(
                    value = state.annualRatePercent, onValueChange = viewModel::onRateChange,
                    label = { Text(stringResource(R.string.rd_label_rate)) },
                    modifier = Modifier.width(140.dp), singleLine = true,
                    trailingIcon = { FieldHelpIcon(stringResource(R.string.help_rd_rate)) }
                )
            }
    ...
    if (state.result != null) {
        ModalBottomSheet(
            ...
            Column(
                ...
                state.result?.let { RdResultCard(it, state) }
...
@Composable
private fun RdResultCard(result: RdResult, state: RdUiState) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        val narrative = buildString {
            append(
                stringResource(
                    R.string.rd_result_narrative,
                    formatAmountWithWords(state.monthlyDeposit.toDoubleOrNull() ?: 0.0, currencyFormat),
                    state.tenureMonths,
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                Text(text = stringResource(R.string.rd_result_maturity_value), style = MaterialTheme.typography.labelLarge)
                Text(
                    text = currencyFormat.format(result.maturityValue),
                    style = MaterialTheme.typography.headlineMedium, fontWeight = FontWeight.Bold
                )
                HorizontalDivider(modifier = Modifier.padding(vertical = 4.dp))
                ResultRow(stringResource(R.string.rd_result_total_deposited), currencyFormat.format(result.totalDeposited))
                ResultRow(stringResource(R.string.rd_result_interest_earned), currencyFormat.format(result.interestEarned))
