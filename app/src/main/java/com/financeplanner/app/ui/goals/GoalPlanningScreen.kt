...
@Composable
fun GoalPlanningScreen(
    onBack: () -> Unit,
    viewModel: GoalPlanningViewModel = hiltViewModel(),
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.goal_planning_title)) },
        ...
        Column(
            ...
            state.error?.let { error ->
                val message = when (error) {
                    is GoalPlanValidationError.InvalidInput -> stringResource(R.string.sip_error_invalid_input)
                    is GoalPlanValidationError.InvalidValue -> error.rawMessage ?: stringResource(R.string.sip_error_invalid_input)
                }
            ...
            AnimatedVisibility(
                ...
                Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    state.result?.let { GoalPlanResultCard(it) }
...
@Composable
private fun GoalPlanResultCard(result: GoalPlanResult) {
